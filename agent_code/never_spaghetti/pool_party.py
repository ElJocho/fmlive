...
def init_pool(current_pool, num_models: int = None, from_file: bool = False):
    ...
    for i in range(num_models):
        ...
        if from_file is False:
            model.get_model().save_weights(os.path.join(pool_path, "model_{}.keras".format(i)))
    if from_file:
...
def mutate_models(self, fitness, current_pool):
    ...
    for i in range(0, len(current_pool)):
        current_pool[i].get_model().save_weights(os.path.join(pool_path, "model_{}.keras".format(i)))

    return current_pool


"""file:///C:/Users/jocho/Downloads/information-10-00390.pdf""" # mutation and crossover choosing